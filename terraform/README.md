...
(dev, prod), all with minimal duplication of code. That's because there is just a single copy of
...
3. Update the `account_name` and `aws_account_id` parameters in [`dev/account.hcl`](/dev/account.hcl) and
...
1. `cd` into the module's folder (e.g. `cd dev/us-east-1/qa/mysql`).
...
1. `cd` into the region folder (e.g. `cd dev/us-east-1`).
